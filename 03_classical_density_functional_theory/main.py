...
outputs = pd.DataFrame(columns=["L", "eta_0", "ST_num", "ST_analytic", "exc_abs"])
...
for i, L in enumerate(Ls):
    ...
    #     ax.plot(range(L-1, N//5 + L-1), rho[(L-1):(N//5 + L-1)], label=rf"$\eta$={eta_0}")
    # ax.set_title(f"L = {L}")
    # ax.set_xlabel("s")
    # ax.set_ylabel("rho")
    # ax.legend()
...
for i, row in df.iterrows():
    ...
    outputs.loc[len(outputs)] = [L, eta_0, ST_num, ST_analytic, exc_abs]
